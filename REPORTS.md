...
- **Visual Aesthetics**: Included Brightness, Contrast, and Colorfulness metrics to capture the visual appeal of creatives beyond semantic embeddings.
- **Tabular Context**: Engineered budget efficiency (budget per day), seasonality (month, day of week), and URL-based features to provide more signal to the regressor.
- **Textual Metadata**: Included text length and word counts to capture the complexity and density of the ad's messaging.
